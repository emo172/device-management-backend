...
- **适用范围**: 项目中的 Java 主代码、测试代码、Mapper XML、SQL 脚本与关键配置文件都必须纳入中文注释治理范围
- **语言要求**: 所有新增注释、补充注释与修订注释统一使用中文（简体），术语必须与本文件、`device_management.sql`、系统设计文档保持一致
- **目标要求**: 注释必须解释“这段代码为什么这样写、对应什么业务规则、有哪些边界条件/副作用/风险点”，禁止把代码字面意思机械翻译成注释
- **同步要求**: 修改代码逻辑、SQL 条件、配置含义、测试断言时，必须同步修改对应注释；代码与注释不一致视为未完成
- **完整要求**: “完整详细中文注释”是指所有具备业务含义、状态约束、维护成本或理解门槛的实现都必须有匹配层级的注释，不要求对显而易见的样板代码做逐行翻译式注释
- **真相源要求**: 注释内容不得与真相源冲突，尤其不得违反三角色模型、审批模式、预约状态、通知模型、定时任务编号、主键 UUID 等核心契约

### 11-2 Java 主代码注释规范

- **类 / 接口注释**: 必须说明职责、所属业务域、在分层架构中的位置、关键依赖与边界约束；例如 Controller 说明承载接口职责，Service 说明业务编排职责，Mapper 说明数据访问职责
- **枚举 / 常量注释**: 必须说明状态码、枚举值或常量的业务含义、适用场景与禁止误用范围，避免不同业务域同码异义被混用
- **字段注释**: 对业务字段、状态字段、时间字段、快照字段、外键/关联字段、布尔控制字段、关键依赖注入字段，必须说明业务语义与使用边界
- **方法注释**: 公共方法必须说明用途、参数含义、返回结果、副作用、异常路径、权限/事务/幂等约束；承载关键业务判断的私有方法也必须说明其规则来源与输出意义
- **逻辑块注释**: 对复杂条件分支、审批链路、状态流转、时间窗口计算、冲突检测、兼容处理、补偿回滚等非直观逻辑，必须使用块级注释说明处理原因与业务依据
...
- **测试类注释**: 必须说明被测对象、覆盖范围与测试目标，是验证业务规则、接口行为还是集成链路
- **测试方法注释**: 必须说明场景前提、测试意图、预期结果与对应风险点，不能只写“测试成功”或“测试失败”这类空泛描述
- **测试数据注释**: 对预约时间窗、审批顺序、逾期时长、冻结阈值、权限身份、状态切换等具有业务含义的测试数据，必须说明为什么这样构造以及它映射哪条业务规则
- **断言注释**: 当断言不是显而易见时，必须说明断言在保护什么业务契约、边界行为或回归风险
- **异常与边界场景**: 权限拒绝、状态非法流转、参数越界、空数据、重复提交、超时处理等测试必须写明场景价值，避免后续维护时误删关键用例

### 11-4 Mapper XML 与 SQL 注释规范

- **Mapper 语句注释**: 每个核心查询、更新、删除、统计语句都必须说明用途、输入条件、输出对象以及对应业务场景
- **动态条件注释**: XML 中的动态 `if`、`choose`、`where`、`set` 条件必须说明触发条件、筛选目的与业务约束，避免维护时误改过滤逻辑
- **关联与映射注释**: 多表关联、结果映射、嵌套查询、分页排序、聚合统计必须说明字段来源、关联关系、排序依据与结果用途
- **SQL 脚本注释**: 建表、索引、初始化数据、迁移脚本必须说明对象职责、字段语义、索引意图、种子数据用途、兼容性影响与风险点
- **性能与一致性注释**: 涉及索引命中、批量更新、统计聚合、逻辑删除、定时扫描、长事务风险的 SQL，必须补充性能或一致性说明
- **术语约束**: SQL 与 XML 注释中必须使用标准业务口径，不得继续使用 `ADMIN`、`PENDING`、`SENT` 等历史旧口径

### 11-5 配置文件注释规范

- **注释策略**: 配置文件采用“块级 / 字段级说明注释”，不要求逐行机械解释，但关键配置块与关键字段必须做到可读、可维护、可追溯
- **必注释配置块**: 数据源、MyBatis / MyBatis-Plus、Spring Security、Redis、JWT、SMTP、AI、定时任务、日志、文件上传等关键配置必须说明用途与影响范围
- **字段说明要求**: 对非直观配置项必须说明含义、默认值影响、可选值范围、与其他配置的联动关系、错误配置可能导致的后果
- **环境差异说明**: 涉及本地、测试、生产环境差异的配置，必须说明覆盖方式、敏感性与部署注意事项
- **敏感信息约束**: 注释可以说明配置用途与风险，但不得在注释中泄露真实密钥、令牌、密码、邮箱授权码等敏感信息
...
- **完成标准**: 新增或修改代码时，若缺少与实现复杂度匹配的中文注释，视为任务未完成
- **评审重点**: 代码审查不仅检查“有没有注释”，还必须检查“注释是否解释了为什么这样做、业务依据是什么、边界条件是什么”
- **重点审查范围**: 业务规则、状态机、审批流、权限控制、事务边界、定时任务、SQL 条件、统计逻辑、外部集成、异常恢复相关改动必须重点审查注释质量
- **文档联动**: 若变更同时影响 schema、接口、状态流转或调度规则，除更新文档外，还必须同步更新代码注释、SQL 注释与配置说明
- **存量治理要求**: 本项目中的存量代码也应按本规范逐步补齐注释；当维护者触达历史代码并进行修改时，应一并完成注释补齐或修正
- **Agent 执行要求**: Agent 在实施开发、修复缺陷、补测试、调整 SQL、修改配置时，必须把补充和校准中文注释作为交付内容的一部分，不得省略
