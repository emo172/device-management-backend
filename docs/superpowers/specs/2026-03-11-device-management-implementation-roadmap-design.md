...
| 阶段 | 目标 | 关键交付 | 主要验证视角 |
|------|------|----------|--------------|
| 阶段 0 | 工程基线与真实 schema 对齐 | Boot 4.x 基线、配置骨架、统一响应异常、安全底座、SQL 脚本、基础枚举 | 仓库可启动、测试基座可运行、代码假设与 SQL 一致 |
...
- 已在 README 与实施计划中同步通知接口正式家族 `/api/notifications*` 和统计接口正式家族 `/api/statistics/*` 的实现口径
- 独立 API 文档仍需继续补齐通知接口与统计接口条目，因此当前状态应视为“仓库内文档已同步”，而非所有外部接口文档都已形成闭环
